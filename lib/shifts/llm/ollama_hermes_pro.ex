defmodule Shifts.LLM.OllamaHermesPro do
  ...
  def generate_next_message(%Chat{llm: {_llm, opts}} = chat) do
    opts =
      opts
      |> Keyword.put(:prompt, use_system(chat.system, chat.tools))
      |> Keyword.update!(:prompt, & use_messages(&1, chat.messages))
  ...
  def get_message(%{"response" => response}) do
    message = case Regex.run(~r/^.*?(?=<tool_call>)/s, response) do
      nil -> Message.new(role: :assistant, content: String.trim(response))
      [prefix] -> Message.new(role: :assistant, content: String.trim(prefix))
    end
        ...
        Enum.reduce(matches, message, fn [_, json], msg ->
          case Jason.decode(json) do
            {:ok, %{"name" => name, "arguments" => input}} ->
              Message.put_record(msg, Tool.tool_use(id: "ollama", name: name, input: input))
            {:error, _error} ->
  ...
  defp use_system(system, tools) do
    system = case system do
  ...
  defp use_messages(system, messages) do
    message_parts =
      ...
      |> Enum.map(fn
        ...
        %Message{role: :assistant, content: content, records: records} ->
          content =
